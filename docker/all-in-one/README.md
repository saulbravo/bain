...
`docker.io/maxivious/bolls:v2.7`
...
```bash
podman build -f docker/all-in-one/Dockerfile -t maxivious/bolls:v2.7 .
podman run --rm -p 8080:80 \
  -v bolls-pgdata:/var/lib/postgresql/data \
  -e AUTO_RESTORE_DB=1 \
  maxivious/bolls:v2.7
```
...
```bash
podman build -f docker/all-in-one/Dockerfile -t docker.io/maxivious/bolls:v2.7 .
podman push docker.io/maxivious/bolls:v2.7
```
...
| `maxivious/bolls:v2.7` all-in-one image | TrueNAS Scale, single-container production |
