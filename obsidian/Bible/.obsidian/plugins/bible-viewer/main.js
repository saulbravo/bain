...
var VERSE_RE = new RegExp(
  `(^|[^A-Za-z0-9\xC1\xC9\xCD\xD3\xDA\xE1\xE9\xED\xF3\xFA\xD1\xF1])(${BOOK_RE})\\s+(\\d{1,3})\\s*:\\s*(\\d{1,3})(?:\\s*[-\u2013\u2014]\\s*(?:(\\d{1,3})\\s*:\\s*)?(\\d{1,3}))?`,
  "giu"
);
function firstVerseOfRange(raw) {
  const match = String(raw || "").match(/^(\d{1,3})(?:\s*[-–—]\s*(\d{1,3}))?$/);
  if (!match) {
    return null;
  }
  const verse = Number(match[1]);
  const endVerse = match[2] ? Number(match[2]) : void 0;
  if (!verse) {
    return null;
  }
  return {
...
function findVerseRefs(text) {
  ...
  while ((match = VERSE_RE.exec(text)) !== null) {
    ...
    const endChapter = match[5] ? Number(match[5]) : void 0;
    const lastNum = match[6] ? Number(match[6]) : void 0;
...
var BibleViewerPlugin = class extends import_obsidian.Plugin {
  ...
  ribbonLabel() {
    return this.settings.paneWidthMode === "full" ? "Bible Viewer: full width (click for half)" : "Bible Viewer: half width (click for full)";
  }
  syncRibbonIcon() {
    var _a, _b;
    if (this.ribbonEl) {
      ...
      (0, import_obsidian.setIcon)(this.ribbonEl, "book-open");
    }
    const leaf = (_a = this.bibleView) == null ? void 0 : _a.leaf;
    if (leaf == null ? void 0 : leaf.tabHeaderInnerIconEl) {
      (0, import_obsidian.setIcon)(leaf.tabHeaderInnerIconEl, "book-open");
      leaf.tabHeaderInnerIconEl.classList.toggle("bible-viewer-half", this.settings.paneWidthMode === "half");
    }
    (_b = leaf == null ? void 0 : leaf.tabHeaderEl) == null ? void 0 : _b.classList.toggle("bible-viewer-half", this.settings.paneWidthMode === "half");
  }
  handleBibleTabIconClick(target, event) {
    if (!target) {
      return false;
    }
    ...
    const header = icon.closest(".workspace-tab-header");
    if (!(header == null ? void 0 : header.classList.contains("is-active"))) {
      return false;
    }
    const leaves = this.app.workspace.getLeavesOfType("bible-viewer");
    for (const workspaceLeaf of leaves) {
      const el = workspaceLeaf.tabHeaderEl;
  ...
  workspaceWidth() {
    ...
    return (el == null ? void 0 : el.clientWidth) || window.innerWidth;
  }
  sideDockSize(dock) {
    var _a;
  ...
  resizableSplit() {
    const leaf = this.app.workspace.getLeavesOfType("bible-viewer")[0];
    let cur = leaf != null ? leaf : null;
    ...
    return this.app.workspace.rightSplit || null;
  }
  applyPaneWidth(mode) {
    var _a, _b, _c;
    const app = this.app;
    ...
    (_a = app.disableCssTransition) == null ? void 0 : _a.call(app);
    ...
    (_c = app.enableCssTransition) == null ? void 0 : _c.call(app);
    this.syncRibbonIcon();
  }
...
var BibleView = class extends import_obsidian.ItemView {
  ...
  highlightStyleKey(el) {
    const style = el.getAttribute("style") || "";
    const fill = this.cssStyleProperty(style, "background-color") || this.cssStyleProperty(style, "background") || this.cssStyleProperty(style, "background-image");
...
var BibleViewerSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Detect verse references").setDesc("Turn written references like Genesis 3:5 or Genesis 1:14-15 into links that open the first verse of the range in Bible Viewer. On by default.").addToggle(
    ...
    new import_obsidian.Setting(containerEl).setName("Open Bible links in Bible Viewer").setDesc("Clicks on Bible links, including ranges like Genesis 1:14-15, open the first verse of the range in Bible Viewer. The note is not changed.").addToggle(
