...
help:
	...
	@echo -e "    "${CYAN}"publish-all-in-one"${RESET}"    Build and push maxivious/bolls:v4.2"
...
build-all-in-one:
	$(DOCKER) build -f docker/all-in-one/Dockerfile -t docker.io/maxivious/bolls:v4.2 .

publish-all-in-one: build-all-in-one
	$(DOCKER) push --format v2s2 docker.io/maxivious/bolls:v4.2
