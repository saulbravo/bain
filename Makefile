...
help:
	...
	@echo -e "    "${CYAN}"publish-all-in-one"${RESET}"    Build and push maxivious/bolls:v2.01"
...
build-all-in-one:
	$(DOCKER) build -f docker/all-in-one/Dockerfile -t docker.io/maxivious/bolls:v2.01 .

publish-all-in-one: build-all-in-one
	$(DOCKER) push docker.io/maxivious/bolls:v2.01
