...
help:
	...
	@echo -e "    "${CYAN}"publish-all-in-one"${RESET}"    Build and push maxivious/bolls:v1.8"
...
build-all-in-one:
	$(DOCKER) build -f docker/all-in-one/Dockerfile -t docker.io/maxivious/bolls:v1.8 .

publish-all-in-one: build-all-in-one
	$(DOCKER) push docker.io/maxivious/bolls:v1.8
